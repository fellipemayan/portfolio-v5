...
export function CustomCursor() {
  ...
  useEffect(() => {
    const moveCursor = (e: MouseEvent) => {
      ...
      const copyEl = target.closest('.copy') as HTMLElement;
      ...
      if (hiddenEl) {
        ...
        const iconPos =
          (textEl.getAttribute('data-cursor-icon-pos') as 'before' | 'after') ||
          'before';

        setText(rawText);
        setIcon(
          rawIcon ? { name: rawIcon, pos: rawText ? iconPos : 'only' } : null
        );
        setCursorState('text');
        if (!rawText && rawIcon) {
        ...
        mouseX.set(
          Math.max(margin, Math.min(targetX, window.innerWidth - margin))
        );
        mouseY.set(
          Math.max(margin, Math.min(targetY, window.innerHeight - margin))
        );
      } else if (copyEl) {
        const rect = copyEl.getBoundingClientRect();
        setCursorState('external');
        setText('');
        setIcon({ name: 'copy', pos: 'only' });
        ...
        const radius = isPill
          ? 999
          : parseFloat(computedStyle.borderRadius) || 8;
  ...
  const contentColor =
    theme === 'black' ? 'var(--color-white)' : 'var(--color-black)';
  ...
  const CurrentIcon =
    icon && ICONS_MAP[icon.name] ? ICONS_MAP[icon.name] : null;
  const showContent =
    cursorState === 'text' ||
    cursorState === 'external' ||
    cursorState === 'icon-only';
